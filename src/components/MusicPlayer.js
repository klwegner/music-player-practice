...
useEffect(() => {
  ...
  if (isPlaying) {
    audioCtx.current.resume().then(playAudio).catch((error) => {
...
useEffect(() => {
  if (currentSong && audioCtx.current) {
    if (!audioSource.current) {
      audioSource.current = audioCtx.current.createMediaElementSource(audioPlayer.current);
...
//   //how many data points we collect from sound--higher number is more bars
//   analyser.fftSize = 128;
//   //always half of fftsize; number of data points we have
// const bufferLength = analyser.frequencyBinCount;
// //holds datapoints collected from sound (holds number of points above)
...
const togglePlay = () => {
  if(!isPlaying) {
    ...
    if (!audioSource.current) {
     ...
     console.log(audioSource.current);
     if (!analyser.current) {
